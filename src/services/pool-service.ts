...
export class PoolService {
    ...
    private simulate(
        balls: Ball[],
        velocities: Phaser.Math.Vector2[]
    ): Phaser.Math.Vector2[][] {
        const friction = 0.98;
        const minVelocity = 0.1;
        const collisionDamping = 0.95;

        const keyPositions: Phaser.Math.Vector2[][] = [
            balls.map((b) => new Vector2(b.phaserSprite.x, b.phaserSprite.y)),
        ];
        ...
        for (let step = 0; step < MAX_STEPS; step++) {
            ...
            keyPositions.push(
                balls.map(
                    (b) => new Vector2(b.phaserSprite.x, b.phaserSprite.y)
                )
            );
